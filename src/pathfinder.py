...
class PathFinder:
    ...
    def _calculate_the_strategic_map(self, end_hub: str) -> Dict[str, int]:
    ...
    def find_path_with_reservations(self, start: str, end: str, start_time: int) -> List[Tuple[str, int]]:
        open_set: List[Tuple[float, int, str, List[Tuple[str, int]]]] = []
        
        initial_h = self.strategic_map.get(start, 999)
        heapq.heappush(open_set, (start_time + initial_h, start_time, start, [(start, start_time)]))
        
        visited: Set[Tuple[str, int]] = {(start, start_time)}

        while open_set:
            _, current_time, curr_name, path = heapq.heappop(open_set)
            ...
            for next_name in possible_moves:
                ...
                move_cost = self._get_move_cost(next_zone, is_waiting)
                ...
                # verificação de Conflitos (Capacidade de Zona e Link) PARTE DO BONUS
                conflict = False
                link_key = self._get_link_key(curr_name, next_name) if not is_waiting else None
                ...
                for time in range(current_time + 1, arrival_time + 1):
                    ...
                    if next_name not in (self.start_hub, self.end_hub):
                        if self.reservations.get((next_name, time), 0) >= next_zone.max_drones:
                            conflict = True; break
                    
                    # capacidade da Ligação (Link)
                    if link_key:
                        max_cap = self.link_capacities.get(link_key, 1)
                        if self.link_reservations.get((link_key[0], link_key[1], time), 0) >= max_cap:
                            conflict = True; break
                ...
                if state not in visited:
                    ...
                    # CÁLCULO DE CUSTO PARA O BÓNUS (45 TURNOS)
                    h_cost = self.strategic_map.get(next_name, 999)
                    priority_bonus = 0.5 if next_zone.zone_type == "priority" else 0.0
                    
                    # Penalização de tráfego para forçar drones a usar caminhos alternativos
                    congestion_factor = self.reservations.get((next_name, arrival_time), 0) * 0.2
                    
                    # f(n) = g(n) + h(n) - bonus + penalty
                    estimated_cost = arrival_time + h_cost - priority_bonus + congestion_factor
                    
                    new_path = path + [(next_name, arrival_time)]
                    heapq.heappush(open_set, (estimated_cost, arrival_time, next_name, new_path))
